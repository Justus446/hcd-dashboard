                ...
                .then(reply => {
                    ...
                    if (reply.fetchedData == undefined || reply.fetchedData?.error) {
                        ...
                        thedxissued.map((issdId, index) => {
                            ...
                            trow.push(reply.fetchedData.metaData.items[issdId].name.replace('MCD_', '').replace('HCD - ','').replace('HIV-', '').replace('Issues', '').replace('TB_FCDRR ', '').replace('Qty Issued by KEMSA','').replace('Facility_TB_2022 ', '').replace('KEMSA_TB','').trim())
                            trow.push(iss_val)
                            recc.map(r_ec => {
                                trow.push(r_ec)
                            })
